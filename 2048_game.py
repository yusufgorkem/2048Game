...
def take_turn(direct, board):
    global score
    merged = [[False for _ in range(len(board))] for _ in range(len(board))]
    if direct == "UP":
        for i in range(4):
            for j in range(4):
                shift = 0
            ...
            for j in range(4):
                ...
                for q in range(i + 1):
                    ...
                    if shift > 0:
                        ...
                        board[2 - i][j] = 0
                    if 3 - i + shift <= 3:
                        if (board[2 - i + shift][j] == board[3 - i + shift] and not merged[3 - i + shift][j]
    ...
    for i in range(4):
        for j in range(4):
            ...
            if value > 0:
                ...
                custom_font = pygame.font.Font("freesansbold.ttf", 48 - (5 * value_len))
                value_text = custom_font.render(str(value), True, value_color)
